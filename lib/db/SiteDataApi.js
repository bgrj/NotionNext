...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  const originalPageRecordMap = await getOrSetDataWithCache(
    ...
    async (pageId, from) => {
      ...
      // Notion API 新格式兼容：若 notion-client 未能识别 collection_view_page 结构
      // 导致 collection_query 为空，则手动补充 queryCollection 调用
      if (
        pageRecordMap &&
        (!pageRecordMap.collection_query ||
          Object.keys(pageRecordMap.collection_query).length === 0)
      ) {
        try {
          await fillCollectionQuery(pageRecordMap, pageId)
        } catch (err) {
          console.warn('[SiteDataApi] fillCollectionQuery failed:', err?.message)
        }
      }
...
/**
 * 当 notion-client 因新版 Notion API block 嵌套格式变化导致 collection_query 为空时，
 * 手动调用 getCollectionData 补充数据
 */
async function fillCollectionQuery(pageRecordMap, pageId) {
  const pageIdUuid = idToUuid(pageId)
  // 从 block 中找到 collection_view_page 的元数据（兼容新旧格式）
  const mainBlockRaw = pageRecordMap.block?.[pageIdUuid]
  if (!mainBlockRaw) return

  // 兼容新版嵌套格式: {spaceId, value: {value: {...}, role}} 和旧版: {value: {...}, role}
  const mainBlockValue =
    mainBlockRaw?.value?.value || mainBlockRaw?.value || mainBlockRaw

  if (
    mainBlockValue?.type !== 'collection_view_page' &&
    mainBlockValue?.type !== 'collection_view'
  ) {
    return
  }

  const collectionId = mainBlockValue.collection_id
  const viewIds = mainBlockValue.view_ids || []
  const spaceId = mainBlockValue.space_id

  if (!collectionId || viewIds.length === 0) return

  pageRecordMap.collection_query = pageRecordMap.collection_query || {}

  for (const collectionViewId of viewIds) {
    // 兼容新版 collectionView 嵌套格式
    const collectionViewRaw = pageRecordMap.collection_view?.[collectionViewId]
    const collectionView =
      collectionViewRaw?.value?.value ||
      collectionViewRaw?.value ||
      collectionViewRaw
    ...
    try {
      ...
      pageRecordMap.collection_query[collectionId] = {
        ...pageRecordMap.collection_query[collectionId],
        [collectionViewId]: collectionData?.result?.reducerResults
      }
    } catch (err) {
      console.warn(
        '[fillCollectionQuery] getCollectionData failed for view:',
        collectionViewId,
        err?.message
      )
    }
...
async function getNotice(post) {
  ...
  // 兼容新版Notion API嵌套格式，确保react-notion-x能正确渲染
...
function handleDataBeforeReturn(db) {
  const tagOptions = cleanTagOptions(db?.tagOptions) || []
  ...
  db.allNavPages = cleanPages(db?.allNavPages, tagOptions)
  db.allPages = cleanPages(db?.allPages, tagOptions)
  db.latestPosts = cleanPages(db?.latestPosts, tagOptions)
  // 必须在使用完毕后才能进行清理
  db.tagOptions = tagOptions
