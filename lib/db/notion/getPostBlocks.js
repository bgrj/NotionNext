...
export async function fetchNotionPageBlocks(id, from = null, slice = 0) {
  ...
  const pageBlock = await getOrSetDataWithCache(
    ...
    async () => {
      // 抓取最新数据
      const recordMap = await getPageWithRetry(id, from)
      return await hydrateMissingBlocks(recordMap)
    }
  )
  ...
  const hydratedPageBlock = await hydrateMissingBlocks(pageBlock)
  if (
    Object.keys(hydratedPageBlock?.block || {}).length !==
    Object.keys(pageBlock?.block || {}).length
  ) {
    await setDataToCache(cacheKey, hydratedPageBlock)
  }

  // 3️⃣ 转换为 post
  return hydratedPageBlock
}
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      if (errorMessage.toLowerCase().includes('page not found')) {
        console.warn('[请求失败-Notion页面不存在]:', `from:${from}`, `id:${id}`)
        return null
      }
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    try {
      ...
      console.warn('[fetchInBatches] getBlocks failed:', String(e?.message || e))
    }
  }
  return fetchedBlocks
}

async function hydrateMissingBlocks(recordMap, maxIterations = 10) {
  if (!recordMap?.block) {
    return recordMap
  }

  let hydratedRecordMap = {
    ...recordMap,
    block: { ...recordMap.block }
  }

  for (let iteration = 0; iteration < maxIterations; iteration++) {
    const missingBlockIds = Array.from(getMissingContentBlockIds(hydratedRecordMap))
    ...
    const fetchedBlockIds = Object.keys(fetchedBlocks || {})
...
function getMissingContentBlockIds(recordMap) {
  const missingBlockIds = new Set()

  for (const blockEntry of Object.values(recordMap?.block || {})) {
    const blockValue = unwrapNestedValue(blockEntry)
...
function unwrapNestedValue(entry) {
  let current = entry
  let guard = 0

  while (current?.value && typeof current.value === 'object' && guard < 5) {
