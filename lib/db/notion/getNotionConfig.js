...
import { fetchInBatches, fetchNotionPageBlocks } from './getPostBlocks'
...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  let content = normalizePageBlock(pageRecordMap?.block?.[configPageId])?.content
  for (const table of ['Config-Table', 'CONFIG-TABLE']) {
    if (content) break
    pageRecordMap = await fetchNotionPageBlocks(configPageId, table)
    content = normalizePageBlock(pageRecordMap?.block?.[configPageId])?.content
  }
  ...
  const configTableId = content?.find(contentId => {
    return normalizePageBlock(pageRecordMap?.block?.[contentId])?.type === 'collection_view'
  })
  ...
  const collection = normalizeCollection(
    pageRecordMap?.collection?.[collectionId] ||
      pageRecordMap?.collection?.[configTableId] ||
      pageRecordMap?.collection?.[rawMetadata?.id]
  )
  ...
  const missingBlockIds = pageIds.filter(id => !block[id])
  ...
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    const temp = normalizePageBlock(block?.[id])
    if (!temp?.properties) {
      continue
    }
