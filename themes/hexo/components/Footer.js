...
const Footer = ({ title }) => {
  ...
  return (
    <footer className='relative z-10 dark:bg-black flex-shrink-0 bg-hexo-light-gray justify-center text-center m-auto w-full leading-6 text-gray-600 dark:text-gray-100 text-sm p-6'>
      ...
      {/* ———— 3. 第三行：版权声明 & 联系方式列表 ———— */}
      {/* 修改点：pb-2 改为 pb-1，缩小下方留白 */}
      <div className='border-t border-gray-200 dark:border-gray-800 pt-4 pb-1 text-xs text-gray-500 dark:text-gray-400 font-sans flex flex-wrap justify-center items-center gap-x-1'>
          ...
          {/* 3. Threads */}
          <a href='https://www.threads.net/@bu.guo.ren.jian' target='_blank' rel='noreferrer' className='hover:underline hover:text-indigo-600'>Threads</a>
          <span className='mx-1.5 text-gray-300'>/</span>
